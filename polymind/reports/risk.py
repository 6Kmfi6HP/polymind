...
def get_risk_report(risk_mgr: RiskManager, limits_mgr: LimitsManager) -> RiskReport:
    ...
    return RiskReport(
        total_exposure=0.0,
        max_exposure=limits_mgr.config.exposure.max_total_exposure
...
def format_risk_table(report: RiskReport) -> Table:
    ...
    dd_color = (
        "green" if report.drawdown_pct < 5 else "yellow" if report.drawdown_pct < 10 else "red"
    )

    table.add_row("Drawdown", f"[{dd_color}]{report.drawdown_pct:.1f}%[/{dd_color}]", health)
    table.add_row(
        "Total Exposure",
        f"${report.total_exposure:.2f}",
        "[green]OK[/green]" if report.total_exposure < report.max_exposure else "[red]OVER[/red]",
    )
    table.add_row(
        "Daily Loss",
        f"${report.daily_loss:.2f}",
        "[green]OK[/green]"
        if report.daily_loss < report.max_daily_loss
        else "[red]LIMIT HIT[/red]",
    )
